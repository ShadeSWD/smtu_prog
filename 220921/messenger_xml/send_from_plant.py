...
class NewQuestion:
    ...
    def output_data(self):
        ...
        dump_questions.dump(Question(idn=self.id, date_and_time=str(self.date_and_time), sender=self.sender,
                                     answered=self.answered, comments=self.comments, text=self.text))
        output = (f'$id={self.id}$time={self.date_and_time}$sender={self.sender}'
                  f'$answered={self.answered}$comments={self.comments}$text={self.text}\n')
